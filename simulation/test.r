...
y <- rnorm(200)
n1 <- f_n1(1:200, k1, m1)
beta <- rnorm(prod(ktau + mtau, k1 + m1))
...
v_dim <- c(ktau + mtau, k1 + m1)
n1 <- f_n1(v_x, k1, m1)
beta <- rnorm(prod(v_dim[1], v_dim[2]))
...
fit <- f_smoothbbgd(v_y, v_x, c(ktau, k1), c(mtau, m1),
        init = NULL,
...
fit <- f_smoothdescent(v_y, v_x, c(ktau, k1), c(mtau, m1),
        init = NULL,
        alpha = 0.1, bet = 0.8,
...
# quantile sheet
dev.new()
system.time(scam_fit <- qs_scam(x, y,
        maxit = 10, ntaus = 10,
        arg_bs = c("tesmi1", "cr"),
        opt = "nlm.fd", dims = c(5, 15), ords = NA,
        analysis = F, trace = T, tol = 1e-3
))
...
dev.new()
v_time <- system.time(gam_fit <- qs_gam(x, y,
        maxit = 50, ntaus = 10,
        dims = c(5, 15), ords = NA,
